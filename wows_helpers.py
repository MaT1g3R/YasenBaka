...
def calc_wtr(expected, actual, coefficients, average_level):
    ...
    ship_frags = actual['frags']/expected['frags'] if expected['frags'] > 0 else 0
...
def player_ship_stats(region, api, id_, ship_list):
    ...
    url = 'https://api.worldofwarships.{}/wows/ships/stats/?application_id={}&' \
          'account_id={}&fields=battles,ship_id&ship_id='.format(region, api, id_)
    res = {}
...
def calculate_coeff():
    ...
    """
    coeff_url = 'https://api.na.warships.today/json/wows/ratings/warships-today-rating/coefficients'
...
def build_embed(region, api, id_, coefficients, expected, ship_dict, ship_list):
    ...
    if stats is not None:
        ...
        nick_url = 'https://api.worldofwarships.{}/wows/account/info/?application_id={}&account_id={}&fields=nickname'\
            .format(region, api, id_)
        nick_name = json.loads(requests.get(nick_url).content)['data'][id_]['nickname']
        battles = all_time_stats['battles']
        all_time_actual = {
            "capture_points": all_time_stats['capture_points'] / battles if battles > 0 else 0,
            "planes_killed": (all_time_stats['planes_killed'] / battles) if battles > 0 else 0,
            "damage_dealt": (all_time_stats['damage_dealt'] / battles) if battles > 0 else 0,
            "frags": (all_time_stats['frags'] / battles) if battles > 0 else 0,
            "dropped_capture_points": all_time_stats[
                                          'dropped_capture_points'] / battles if battles > 0 else 0,
            "wins": (all_time_stats['wins'] / battles) if battles > 0 else 0
        }
        average_tier = calculate_average_tier(region, api, id_, ship_dict, ship_list)
        all_time_wtr_val = calc_wtr(expected, all_time_actual, coefficients, average_tier)
        ...
        if battles > 0:
            ...
            eb.add_field(name='All Time Stats', value=str(battles) + ' Battles', inline=False)
            ...
            if recent_stats is not None:
                ...
                recent_actual = {
                    "capture_points": recent_stats['capture_points'] / recent_battles if recent_battles > 0 else 0,
                    "planes_killed": recent_stats['planes_killed'] / recent_battles if recent_battles > 0 else 0,
                    "damage_dealt": recent_stats['damage_dealt'] / recent_battles if recent_battles > 0 else 0,
                    "frags": recent_stats['frags'] / recent_battles if recent_battles > 0 else 0,
                    "dropped_capture_points": recent_stats[
                                                  'dropped_capture_points'] / recent_battles if recent_battles > 0 else 0,
                ...
                eb.add_field(name='Average Kills', value=str("{0:.2f}".format(recent_actual['frags'])))
        else:
